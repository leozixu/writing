...
from pywebio.output import put_text, put_markdown, put_success, put_file, put_scrollable
...
# === 双向日志输出系统 ===
# =============================
log_box = None  # 保存网页端输出框

def show_log(msg):
    """在网页端显示日志"""
    global log_box
    if log_box:
        log_box.append(put_text(msg))
...
async def main():
    ...
    # === 显示日志输出区 ===
    put_markdown("## 🧾 实时运行日志")
    log_box = put_scrollable(content=[], height=300, keep_bottom=True)
    put_markdown("---")
    ...
    print(f"✅ PDF {first_pdf['filename']} 上传成功，开始提取图片和章节信息...")
    ...
    print("✅ 图片信息提取完成，并已保存为 image_report.json")
    ...
    put_file(img_zip_filename, img_memory_file.read(), "下载参考图片及信息")
    print("✅ 已打包图片信息供下载")
